    ...
    def __init__(self,parent,select,enlargement=1,colour='red',unit='Temp C'):
        ...
        for j in range(0,scale_extent//unitspertick+1):
            ...
            if j % 10==0:
                ...
                tk_text(self,tangle,ce,re-x*e,angle,fill=fdial,font=dfont,size=10*e)

        if max_value != 1023:
            tk_text(self,'888',ce,0,0,fill=gdial,font=dfont,size=60*e)
        else:
            tk_text(self,'8888',ce,0,0,fill=gdial,font=dfont,size=45*e)

        y=(re*4//10 if max_value in (255,1023) else re//2)
        tk_text(self,mess,(ce[0],ce[1]-y),0,0,
            fill=fdial,font=dfont,size=10*e)
...
if __name__ == '__main__':
    ...
    l2=LCD(fr,select=4,enlargement=2,colour='blue',unit='Luminosity')
