...
def draw_pointer(canv,x0,y0,degree, t,r):
    this_color = "#00A2E8" # wedgewood like blue
    xr=x0
    yr=y0
...
class DrawGauge2(ini):
    ...
    def draw_tick(self,divisions=100):
        ...
        label = self.unit
        self.canvas.create_text(self.size/2,2*self.size/5, font=("Arial",
            int(self.size/20)),fill="white", text=label,angle=0)
        label = 'Ardiotech'
        self.canvas.create_text(self.size/2,3*self.size/5, font=("Arial",
            int(self.size/18),'bold'),fill="light blue", text=label,angle=0)
        ...
        for tick in range(divisions+1):
            ...
            if (tick%10) == 0:
                ...
                # writing dial numbers
                self.canvas.create_text(x,y, font=("Arial",int(self.size/25)),fill="white", text=label) # fill="white"
                # print(label,x,y)
    ...
    def set_value(self, number: (float, int)):
        ...
        draw_pointer(self.canvas,self.size/2,self.size/2,-1*degree,self.size/3,8)
        label = str('%.2f' % number)
